...
Then, in an attempt to garner a deeper understanding of how the US compares to other nations with regards to their COVID-19 schooling policies and measures, we employ @tbl-daysclosed which provides a snippet of the top 5 countries by number of school closures between the start of January 2020 and the end of December 2021. Due to the US featuring the second-highest number of school closures, @tbl-daysclosed specifically depicts the top 5 nations in an attempt to evidence the US’ large number of closures relative to the rest of the world. In order to generate this figure, we employed the same cleaned dataset used to create @fig-daysclosed. In this particular case, we simply grouped the data by country and summed the number of instances where a country registered a Covid Tracker score of 2 or 3 and reported a snippet of the generated table detailing each country and the number of school closures it faced in the aforementioned time frame.
